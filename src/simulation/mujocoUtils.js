...
export async function reloadPolicy(policy_path, options = {}) {
  ...
  // Wait for all PolicyRunner inference to finish.
  const isMultiRobot = this.robotConfigs && this.robotConfigs.length > 1;
  // If switching back to single-robot mode, clear multi-robot state to avoid stale runners/mappings.
  if (!isMultiRobot) {
    this.policyRunners = [];
  ...
  // Detect multi-robot mode using the isMultiRobot flag.
  if (isMultiRobot) {
    // Multi-robot: build mappings for each robot.
    this.robotJointMappings = [];
    ...
    // Also update the backward-compatible single-robot arrays from robot 0 mapping.
  ...
  // Detect multi-robot mode using the isMultiRobot flag.
  if (isMultiRobot) {
    // Multi-robot: create one PolicyRunner per robot.
    // Clear old policyRunners (if any).
    if (this.policyRunners) {
      for (const runner of this.policyRunners) {
        // PolicyRunner has no explicit cleanup; it will be GC'd.
      }
    }
    this.policyRunners = [];
    ...
    // Backward compatibility: keep robot 0 runner in this.policyRunner as well.
...
/**
 * Reload policy for a single robot (foundation for independent per-robot policies).
 *
 * Notes:
 * - The main loop still branches on the global this.control_type in multi-robot mode.
 *   Therefore all robots must use the same control_type, otherwise we throw.
 * - PD gains can be read per robot via this.robotPolicyParams[robotIdx].kp/kd.
 */
export async function reloadPolicyForRobot(robotIdx, policy_path, options = {}) {
  ...
  const isMultiRobot = this.robotConfigs && this.robotConfigs.length > 1;
  if (!isMultiRobot) {
    // Single robot: fall back to full reload.
  ...
  // Wait for this robot's runner to finish inference (avoid concurrency issues).
  ...
  // Update joint mapping for this robot (different policies may have different joint lists).
  ...
  // Create a dedicated PolicyRunner for this robot.
  const policyRunner = new PolicyRunner(
    {
      ...config,
      tracking: trackingConfig,
  ...
  // Store per-robot policy params for the main loop.
  ...
  // Update config record (for UI/debug display).
...
export async function loadSceneFromURL(mujoco, filename, parent) {
  ...
  for (let g = 0; g < model.ngeom; g++) {
    ...
    if (type == mujoco.mjtGeom.mjGEOM_PLANE.value) {
      ...
      // Heightfields are not rendered yet; keep a small placeholder geometry.
...
/**
 * Configure joint mappings for a specific robot (multi-robot support).
 * Reuses configureJointMappings logic but supports joint name prefixes.
 * @param {MuJoCoDemo} demo - MuJoCoDemo instance
 * @param {Array<string>} jointNames - policy joint name list (raw names, no prefix)
 * @param {string} prefix - robot prefix (e.g. 'robot2_' or '')
 * @param {number} robotIndex - robot index
 */
function configureJointMappingsWithPrefix(demo, jointNames, prefix, robotIndex) {
  const model = demo.model;
  const mujoco = demo.mujoco;
  
  // Build the prefixed joint name list.
  const prefixedJointNames = jointNames.map(name => prefix + name);
  
  // Build actuator->joint mapping (same as configureJointMappings).
  const jointTransmission = mujoco.mjtTrn.mjTRN_JOINT.value;
  const actuator2joint = [];
  for (let i = 0; i < model.nu; i++) {
    if (model.actuator_trntype[i] !== jointTransmission) {
      continue; // skip non-joint actuators
    }
    actuator2joint.push(model.actuator_trnid[2 * i]);
  }
  ...
  // Build mapping for each policy joint.
  for (let i = 0; i < prefixedJointNames.length; i++) {
    ...
    // Find the corresponding actuator.
    const actuatorIdx = actuator2joint.findIndex((jointId) => jointId === jointIdx);
    if (actuatorIdx < 0) {
  ...
  // Find this robot's freejoint addresses (prefer recorded robotPelvisBodyIds).
  let pelvisBodyId = -1;
  
  // Prefer recorded robotPelvisBodyIds (found in setMultiRobotInitialPositions).
  if (demo.robotPelvisBodyIds && demo.robotPelvisBodyIds[robotIndex] !== undefined) {
    pelvisBodyId = demo.robotPelvisBodyIds[robotIndex];
  } else {
    // Fallback: search again if robotPelvisBodyIds is missing.
  ...
  if (pelvisBodyId >= 0) {
    for (let j = 0; j < model.njnt; j++) {
      if (model.jnt_bodyid[j] === pelvisBodyId) {
        ...
        if (qposAdr >= 0) {
          // Store freejoint addresses in the mapping (pelvis typically has a single freejoint).
  ...
  // If freejoint wasn't found, use defaults.
  ...
  // Store into robotJointMappings array.
